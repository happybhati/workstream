...
def _extract_existing_info(scan: dict) -> dict:
    ...
    test_re = re.compile(r"(?i)(go\s+test|pytest|npm\s+test|cargo\s+test|" r"make\s+test|yarn\s+test|mvn\s+test)")
...
def generate_skill_running_e2e_tests(scan: dict) -> str:
    ...
    e2e_paths = [
        p for p in all_paths if any(seg in p.lower() for seg in ("e2e", "integration-test", "integration_test"))
    ]
    has_kind = any("kind" in p.lower() for p in all_paths)
    has_docker_compose = any(
        "docker-compose" in p.lower() or "compose.yaml" in p.lower() or "compose.yml" in p.lower() for p in all_paths
    )
...
def generate_skill_local_dev_setup(scan: dict) -> str:
    ...
    if "python" in lang:
        prereqs += [
            "- [Python 3](https://www.python.org/downloads/) (check `pyproject.toml` or `setup.cfg` for minimum version)"
        ]
